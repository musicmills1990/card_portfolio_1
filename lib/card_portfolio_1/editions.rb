...
def self.scrape_editions
  ...
  editions << self.scrape_site_editions
  editions
end

def self.scrape_site_editions
doc = Nokogiri::HTML(open("https://musicthegathering.com"))
editions = self.new
editions.name = doc.search("h1.inner-content has-content.text-align-center").text #probably not working but I can't get my pry working
editions
end
